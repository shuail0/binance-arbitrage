...
币安交易策略 Python 项目，从 OKX 项目 (okx-arbitrage) 移植。
使用官方 SDK (`binance-sdk-spot` + `binance-sdk-derivatives-trading-usds-futures`) 对接币安 API，认证/签名/重连由 SDK 自动处理。
...
认证方式: Ed25519 私钥签名（PEM 文件），非 HMAC。SDK 自动处理签名，无需手动实现。
...
- SDK WS 回调接收 Pydantic 模型，用 `getattr(msg, field, None)` 访问字段
- 事件循环: `asyncio.get_running_loop()` 而非已弃用的 `get_event_loop()`
- 共用客户端工厂: `strategies/common/sdk_clients.py` (`load_credentials`, `make_spot_client`, `make_futures_client`)
- WS 错误解析: `strategies/common/ws_errors.py` (`parse_ws_error`, `is_post_only_reject`, `is_order_gone`)
